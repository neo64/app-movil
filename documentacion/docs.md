...
- Si el token caduca, la app automaticamente te cierra la sesion y te pide que te loguees. al logarte automaticamente crea un nuevo token
...
- Si el envio de una notificación falla porque el usuario a desinstalado la app, Firebase devuelve el error "NotRegistered" Y nosotros
...
- Cada vez que un usuario abre la app, esta llama a la api al método getTimeServer, este comprueba si el token esta o no expirado. Si no está expirado cambia la fehca de expiración a dentro de un mes. De tal manera que si un usuario es activo no le caduca el token.
